        ...
        while True:
            ...
            if self.draw_line_data:
                if cfg.show_window and cfg.show_target_line:
                    cv2.line(self.image, (capture.screen_x_center, capture.screen_y_center),
                             (int(self.draw_line_data[0]), int(self.draw_line_data[1])), (0, 255, 255), 2)

                if cfg.show_overlay and cfg.overlay_show_target_line:
                    overlay.draw_line(capture.screen_x_center, capture.screen_y_center, int(self.draw_line_data[0]),
                                      int(self.draw_line_data[1]), 'green', 2)
            ...
            if self.draw_boxes_data is not None:
                ...
                for xyxy, cls, conf in zip(xyxy_iter, cls_iter, conf_iter):
                    ...
                    if cfg.show_window and cfg.show_labels and not cfg.show_conf:
                        cv2.putText(self.image, str_cls, (x0, y0 - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 200, 0), 1,
                                    cv2.LINE_AA)
                    ...
                    if cfg.show_window and cfg.show_conf:
                        cv2.putText(self.image, conf_text, (x0, y0 - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 200, 0), 1,
                                    cv2.LINE_AA)
            ...
            if self.draw_speed_data:
                if cfg.show_window:
                    cv2.putText(self.image, 'preprocess: {:.2f}'.format(self.draw_speed_data[0]), (10, 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1, cv2.LINE_AA)
                    cv2.putText(self.image, 'inference: {:.2f}'.format(self.draw_speed_data[1]), (10, 40),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1, cv2.LINE_AA)
                    cv2.putText(self.image, 'postprocess: {:.2f}'.format(self.draw_speed_data[2]), (10, 60),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1, cv2.LINE_AA)
            ...
            if cfg.show_window_fps:
                ...
                if cfg.show_window:
                    cv2.putText(self.image, f'FPS: {str(int(fps))}', (10, 80) if cfg.show_detection_speed else (10, 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1, cv2.LINE_AA)
            ...
            if self.draw_bScope_data:
                ...
                if bScope:
                    if cfg.show_window:
                        cv2.putText(self.image, 'True', (int(b_x1), int(b_y1) - 5), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (200, 0, 0), 2, cv2.LINE_AA)
                else:
                    if cfg.show_window:
                        cv2.putText(self.image, 'False', (int(b_x1), int(b_y1) - 5), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (200, 0, 0), 2, cv2.LINE_AA)
            ...
            if self.draw_predicted_position_data:
                if cfg.show_window and cfg.show_target_prediction_line:
                    cv2.line(self.image, (capture.screen_x_center, capture.screen_y_center),
                             (int(self.draw_predicted_position_data[0]), int(self.draw_predicted_position_data[1])),
                             (255, 0, 255), 2)

                if cfg.show_overlay and cfg.overlay_show_target_prediction_line:
                    overlay.draw_line(capture.screen_x_center, capture.screen_y_center,
                                      int(self.draw_predicted_position_data[0]),
                                      int(self.draw_predicted_position_data[1]), 'green', 2)
            ...
            if self.draw_history_point_data:
                ...
                for x, y in self.draw_history_point_data:
                    if cfg.show_window:
                        cv2.circle(self.image, (x + capture.screen_x_center, y + capture.screen_y_center), radius=1,
    ...
    def spawn_debug_window(self):
        ...
        if cfg.debug_window_always_on_top:
            try:
                ...
                win32gui.SetWindowPos(debug_window_hwnd, win32con.HWND_TOPMOST, x, y, cfg.detection_window_width,
                                      cfg.detection_window_height, 0)
            except Exception as e:
                logger.error(
                    f'[Visuals] Error with on top window, skipping this option `debug_window_always_on_top`: {e}')
